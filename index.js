 /*
* Bot es una creación de Eriz0o
* Bot no tiene ningun fin de lucro
* erizo se reserva todos los derechos de autor
* © 2021 Eriz0o, INC.
...
function addMetadata(packname, author) {	
	if (!packname) packname = '𝐂𝐮𝐫𝐬𝐞𝐝 𝐂𝐨𝐦𝐦𝐮𝐧𝐢𝐭𝐲'; if (!author) author = 'Ᏼo͢Ꭲ';	
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Bienvenid@ a *${mdata.subject}* la comunidad mas insolente de internet\n\nPara utilizar el bot registrate con el comando ${prefix}daftar y tu nombre\n\nPara ver los demas comandos utiliza ${prefix}help\n\nSe recomienda desactivar las descargas automaticas`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Se salio un gei @${num.split('@')[0]}\n\nQue dios lo bendiga `
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo uso yo',
					admin: '[❗] Este comando solo puede ser utilizado por administradores del grupo',
					Badmin: '[❗] Necesito Ser Admin Para Este Comando',
                                        pegatina: 'En Proceso, Aguarda!\n\n*Recuerda los stickersgif son de 6 segundos ❗*',
					attp: 'Convirtiendo texto a sticker\n\n*Esto puede demorar unos segundos*',
					imgs: '*Espera! Estoy creando tu imagen!*',
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido, adios : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
